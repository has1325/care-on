      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" data-testid="link-logo">
            <span className="text-2xl font-bold text-primary tracking-tight">케어링크</span>
          </Link>

          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} data-testid={`nav-${link.label}`}>
                <span
                  className={cn(
      ...
      {open && (
        <div className="md:hidden border-t border-border bg-white px-4 py-3 space-y-1">
          ...
          <div className="flex gap-2 pt-2">
            <Link href="/emergency" className="flex-1" onClick={() => setOpen(false)}>
              <Button variant="destructive" size="sm" className="w-full gap-1.5">
                <AlertTriangle className="w-3.5 h-3.5" />
                긴급 요청
              </Button>
            </Link>
            <Link href="/consultation" className="flex-1" onClick={() => setOpen(false)}>
